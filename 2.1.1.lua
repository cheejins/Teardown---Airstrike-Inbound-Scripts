...
--[[Planes]]
    ...
    function checkAirstrikeState(plane)
        ...
        if VecLength(VecSub(planeTransform.pos, plane.targetRaycastPos)) < plane.adjustedSpawnDistance then 
            plane.isActive = true
        else
            plane.isActive = false
        end
        ...
        if plane.isActive == false and plane.airstrikeEnded == false then
            plane.endAirtrike() -- called one frame only
        end
    end
    ...
    function designateTarget(plane)
        if InputPressed("lmb") then
            if plane.isActive == false and GetString("game.player.tool") == plane.tool and raycastFromPlayerTransform() ~= nil then -- player is looking at a valid voxel while plane ready

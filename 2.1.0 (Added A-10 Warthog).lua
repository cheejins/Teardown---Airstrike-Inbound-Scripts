...
local sounds = {
    ...
    planeIsReady     = LoadSound("valuable0"),
    ...
    function constructDefaultPlane(name, tag, tool)
        ...
        --[[Variables]]
            ...
            -- transform
                ...
                plane.bounds            = {-600, 600}
                if db.all then
                    plane.speed             = 25
                    plane.spawnHeight       = 25
                    plane.bounds            = {-75, 75}
                end
                plane.boundsDistance    = plane.bounds[2]
                plane.spawnDistance     = plane.bounds[2]
            ...
            -- UI
                ...
                plane.notificationTimerDefault      = 0.8
       ...
       --- Spawn point and transform of the plane when airstrike designated.
           function()
            ...
            --- Scroll mouse up or down (change weapon) to reset target pos values before plane starts airstrike.
                function()
                    if (InputValue("mousewheel") > 0 or InputValue("mousewheel") < 0) and plane.isReady == true 
                    and plane.isActive == false then
                        plane.targetRaycastPos = nil
                        plane.targetEndRaycastPos = nil
                        plane.smokeIsOn = false
                    end
            ...
            plane.airstrikeFinished =
            --- Sound plays once when plane becomes ready for another airstrike.
                function()
                    if plane.isReadySoundPlayed == false then 
                        PlaySound(sounds.planeIsReady, GetPlayerPos(), 1)
                        plane.isReadySoundPlayed        = true
                        plane.smokeIsOn                 = false
                        plane.smokeTimer                = 0
                        plane.airstrikeEnded            = true
                        ...
                        plane.notificationTimer         = plane.notificationTimerDefault
                        plane.proxyPlayerPos            = Vec(0,-1000,0)
            ...
            plane.drawPathLines =
            --- Draws lines between the key position points of the airstrike.
                function()
                        ...
                        if plane.pathLineTimer > 0 then
                            ...
                            if plane.name == "a10" then
                                ...
                                local playerRaycastPosTargetEndHeightPos = VecAdd(plane.targetEndRaycastPos, 
                                    Vec(0,plane.targetRaycastPos[2]+(CalcDistance(plane.targetRaycastPos,plane.targetEndRaycastPos))/2,0))

                                local lineSpacing = CalcDistance(plane.targetEndRaycastPos,plane.targetRaycastPos)
            ...
            --- Creates a preview line after the first click, based on the player's raycast position, offsetted height.
                function()
                    local lineColor = colors.yellow

                    if raycastFromPlayerTransform() ~= nil and plane.isActive == false then
                        local playerRaycastCopy = raycastFromPlayerTransform()
                        
                        if plane.name == "f15" then
                            local playerRaycastPosTargetHeight = Vec(playerRaycastCopy[1], plane.targetRaycastPos[2], playerRaycastCopy[3])
                            ...
                            local endPosFloor = TransformToParentPoint(d, Vec(0,-playerRaycastPosTargetHeight[2], dist))
                        ...
                        if plane.name == "a10" then
                            local playerRaycastPosTargetEndHeightPos = VecAdd(playerRaycastCopy, Vec(0,plane.targetRaycastPos[2]+(CalcDistance(plane.targetRaycastPos,VecAdd(playerRaycastCopy)))/2,0))

                            local lineSpacing = CalcDistance(playerRaycastCopy,plane.targetRaycastPos)
                            for i=1, lineSpacing do
            ...
            --- Decides which plane to use based on which weapon is equipped.
                function()
    ...
    -- A10 Config (changing these settings might break the mod)
        local a10 = constructDefaultPlane("a10","airstrike_a10", "blowtorch")
            if db.all then
                ...
                a10.bounds    = {-800, 800} -- second number controls spawn distance
            end
            ...
            a10.boundsDistance          = a10.bounds[2]
            a10.spawnDistance           = a10.bounds[2]
...
--[[Plane Functions]]
    ...
    function tick()        
        ...
        local planes = {a10, f15}
        if ready_planes_notifications then planeNotification(planes) end

        -- checkedPlaneInMap
            -- if checkedPlaneInMap == false then
            --     if FindBody(f15.tag) == nil or FindBody(a10.tag) == nil then 
            --         checkedPlaneInMap = true
            --     end
            -- end
            -- SetString("hud.notification", "ERROR: Plane vox files or plane xml invalid/missing! Mod will not work unless both are installed properly.")
    end
    function manageAirstrike(plane)
        checkPlaneReadyState(plane)
        ...
        if plane.checkPlayerTool() then
            -- DebugPrint(GetBodyTransform(plane.body).pos[1] .. " ---  " ..  GetBodyTransform(plane.body).pos[2] .. " ---  " ..  GetBodyTransform(plane.body).pos[3])
            -- DebugPrint(GetBodyTransform(plane.body).rot[1] .. " ---  " ..  GetBodyTransform(plane.body).rot[2] .. " ---  " ..  GetBodyTransform(plane.body).rot[3])
        end
    ...
    function checkPlaneReadyState(plane) -- plane ready based on its location
        local planeTransform = GetBodyTransform(plane.body)

        if VecLength(VecSub(planeTransform.pos, plane.targetRaycastPos)) 
        < math.sqrt(plane.spawnDistance^2 + plane.spawnHeight^2 + (30 or plane.raycastFromPlayerTransform[2])) * 1.1 then -- plane hypotenuse of height from target point.

            plane.isReady = false
            plane.isActive = true
            plane.isReadySoundPlayed = false
            plane.sound_bombsDroppedHasPlayed = false

            -- SetCameraTransform(Transform(
            --     TransformToParentPoint(planeTransform,Vec(0,10,15)), 
            -- planeTransform.rot))

        else -- plane has passed airstrike zone
            -- reset plane values for next airstrike
            plane.airstrikeFinished()
            plane.isReady = true
            plane.isActive = false            
            plane.sound_bombsDroppedHasPlayed = false
    ...
    function designateTarget(plane)
        if InputPressed("lmb") then
            if plane.isReady and GetString("game.player.tool") == plane.tool and raycastFromPlayerTransform() ~= nil then -- player is looking at a valid voxel while plane ready

                plane.airstrikeEnded = false

                local planeTransform = GetBodyTransform(plane.body)
                local playerRaycastPos = raycastFromPlayerTransform()
                local playerPos = GetPlayerPos()

                -- target pos link setup
                -- if plane.targetRaycastPos == nil then -- set first target point
                --     plane.targetRaycastPos = playerRaycastPos
                -- elseif plane.targetEndRaycastPos == nil then -- first point never nil before second
                --     plane.targetEndRaycastPos = playerRaycastPos
                -- end
                plane.setTargetRaycastPos()

                plane.smokeIsOn = true
                plane.smokeTimer = 100   -- seconds on
            ...
            elseif plane.isReady == false and GetString("game.player.tool") == plane.tool then
                PlaySound(sounds.planeNotReady, GetPlayerPos(), 10)  -- play not ready sound.
            end


            if plane.isReady then
    ...
    function planeSound(plane)
        ...
        local planeTransform = GetBodyTransform(plane.body)
        -- local distanceJetToPlayer = CalcDistance(planeTransform.pos, GetPlayerPos())
        ...
        if plane.distancePlaneToPlayer > plane.minimumDistanceToPlayer and plane.isActive then -- proxyplayer outgoing
    ...
    function planeShoot(plane)
        ...
        if plane.isActive then
            ...
            if plane.name == "f15" then
                ...
                if CalcDistance(plane.proxyPos, plane.targetRaycastPos) < plane.shootPrefireDistance then
            ...
            elseif plane.name == "a10" then
                ...
                and plane.isDoneShooting == false then
                    ...
                    if plane.shootTimer > 0 then -- shoot time in seconds
                        ...
                        -- if plane.shootTimer < plane.shootTimerDefault*0.9 then -- simulate bullet velocity hit delay
    ...
    function shoot(plane, spread)
        ...
        if plane.name == "f15" then
            ...
            if plane.bombSpread <= 0 then  
                d[2] = d[2] + plane.shootPrefireDistance/42 -- + plane.targetRaycastPos[2]/150 -- very slight y up-tilt to adjust for rocket projectile drop.
            else
                d[2] = d[2] + (math.random()-0.5)*spread + plane.shootPrefireDistance/42
            end
